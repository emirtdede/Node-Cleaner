...
export interface VelliumLogoProps {
  size?: number;
  width?: number | string;
...
export const VelliumLogo: React.FC<VelliumLogoProps> = ({
  size = 32,
  width,
  height,
  className = "",
  ...
  const finalHeight = height || (typeof finalWidth === "number" ? Math.round(finalWidth * 0.8) : finalWidth);

  return (
    <div
      className={`vellium-logo-container ${className}`}
      style={{
        display: "inline-flex",
        alignItems: "center",
        gap: 12,
